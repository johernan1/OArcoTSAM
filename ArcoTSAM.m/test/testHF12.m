...
%
% Cálculo para una distribución de carga uniforme en el spoke (como hace
% H*)
% La solucion de H* es INSEGURA para su distribución de carga:
%     rint->0:  H*=23.6;  HLP=27.18    (27.18-23.6)/23.6=15.2%
%     r=0.5     H*=23.6;  HLP=23.09    (23.09-23.6)/23.6=2.16% 
%
% Cálculo para una distribución triangular de carga en el spoke. 
% Correccion de la solución de H*   qL2/8->qL2/12
%     rint->0:  H*=15.7;  HLP=18.69    (18.69-15.7)/15.7=19.0% INSEGURO
%     r=0.5:    H*=15.7;  HLP=18.14    (18.14-15.7)/15.7=15.5% INSEGURO
%
% Discusión. La solución de H* es INSEGURA aceptando su distribución de
% carga. Es segura si se compara con la solución de LP para una
% distribución triangular de carga. Si se calcula la solución de H* para
% esa misma distribución de caga la solución es MUY INSEGURA (approx 20%)
% Por pura casualidad se obiene el mismo H si se considera un 'óculo' de 
% radio 0.5. Pero es pura casualidad. En ningún caso este resultado puede
% validar la propuesta de H* 
%
% TODO:
% Perturbación en w
...
radioI= 0.5;    % radio del inner ring
...
qpmlH=w*(AreaT-AreaI)/nSpoke/(radioT-radioI)

%qpmlSpoke=qpmlH*cos(angulS/2);
%qpmlPetal=qpmlH*cos(angulP)/2;
qpmlSpoke=w*(AreaT-AreaI)/nSpoke/(radioT-radioI);
...
fprintf('%21f|%10f|%17f|%10f|%12f\n', ...
    qpmlH, qpmlSpoke, qpmlH*(2*radioT)^2/8/t,  minhdirP*cos(pi/6), qpmlH*(2*radioT)^2/8/t/(minhdirP*cos(pi/6)));
...
fprintf('%21f|%10f|%18f|%10f|%12f\n', ...
    qpmlH, qpmlSpoke, qpmlH*(2*radioT)^2/12/t,  minhdirP*cos(pi/6), qpmlH*(2*radioT)^2/12/t/(minhdirP*cos(pi/6)));
%% DIBUJO Resultados
%
...
Srv=0;
for jspoke=1:nSpoke
    Srv=Srv+MRBpeta1{jspoke}.elems{neleSp}.VectS{3}(6);
    Srv=Srv+MRBpeta2{jspoke}.elems{neleSp}.VectS{3}(6);
end
Sq=w*AreaT;
