...
% Variación. Se modifica la carga sobre dos de los spokes manteniendose el
% total
clear all;
...
neleS= 18; % numero elementos de cada spoke 
...
for ie=1:MRBa.GetNelems
    ...
    MRBb.SetQApml(ihip, ie,4, [0 qpml*2 0]);
    MRBc.SetQApml(ihip, ie,4, [0 qpml*0 0]);
end
vectQ=MRB.Getf(ihip);
...
%% Chequeos
%
Srv=-MRBa.elems{1}.VectS{1}(3)+0*MRBa.elems{MRBa.GetNelems}.VectS{1}(6) ...
    -MRBb.elems{1}.VectS{1}(3)+0*MRBb.elems{MRBa.GetNelems}.VectS{1}(6) ...
    -MRBc.elems{1}.VectS{1}(3)+0*MRBc.elems{MRBa.GetNelems}.VectS{1}(6);
Sq=qpml*lS*3;    
...
fprintf('Reacción vertical en spoke 2: (%f)\n', MRBb.elems{1}.VectS{1}(3));

fprintf('Reacción horizontal en spoke 3: (%f)\n',   MRBc.elems{1}.VectS{1}(2));
